...
def msa_generate(args, model, dataset, msa_collator, tokenizer):
    ...
    with torch.no_grad():
        ...
        for a3m_data in dataset:
            ...
            input_ids = [seq[1] for seq in msa]
            src_ids = msa_collator.msa_batch_convert(input_ids).to(args.device)
            ...
            for trial in range(args.trials_times):
                ...
                generate_seq = list(filter(lambda x: len(set(x)) >= 4 and len(x) == len(msa[0][1]), generate_seq))
...
def inference(args):
    ...
    tokenizer = Alphabet.from_architecture(name="msa_transformer", theme="rna-3-mixmer")
