    ...
    def forward(
        ...
        if self.dec_col_attention or self.enc_col_attention: 
            if query_states.size(3) != key_states.size(3):#generate
                ...
                scores = torch.einsum("bincd,bjncd->bncij", query_states, key_states) #(br_1nqd)*(br_2nkd)->(bnqr_1r_2) requires matching seq lengths
        ...
        if position_bias is None: 
            ...
            else:  # Compute position_bias for the first layer
...
class T5LayerAxialAttention(nn.Module):
    ...
    def forward(
        ...
        if position_bias is None:
            position_bias = attention_output_row[2] # [attention_output, present_key_value_state, position_bias]
        # Column Attention
        normed_hidden_states=self.layer_norm2(hidden_states_row)
        attention_output_col = self.SelfColumnAttention(
            normed_hidden_states,
            mask=attention_mask,
            position_bias=position_bias,  # Must not be None for column attention
            layer_head_mask=layer_head_mask,
            past_key_value=past_key_value,
            use_cache=use_cache,
            output_attentions=output_attentions,
...
class PSSMWeightedCELoss(nn.Module):
    ...
    def calculate_pssm_weights(self, labels, ignore_index=-100):
        """Compute normalized PSSM weights for each position."""
        ...
        for b in range(batch_size):
            ...
            if len(valid_weights) > 0:
                normalized_weights = conservation_scores[b]  # No extra normalization
                normalized_weights = normalized_weights.clamp(min=0.5, max=1.5)  # Clamp weights to [0.5, 1.5]
                normalized_weights = normalized_weights * mask[b, 0].float() + (~mask[b, 0]).float()  # Invalid positions get weight 1
            else:
                normalized_weights = torch.ones_like(conservation_scores[b])  # If nothing is valid, fall back to all ones
    ...
    def forward(self, lm_logits, labels):
        if labels is not None:
            ...
            pssm_weights = pssm_weights.reshape(-1)  # Flatten to match ce_loss shape
...
class MSA_AUGMENTOR(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = T5Stack(encoder_config, self.shared, self.esm_input) # gx
    ...
    @add_start_docstrings(PARALLELIZE_DOCSTRING)
    def parallelize(self, device_map=None):  # Distribute layers across devices
        self.device_map = (
    ...
    def forward(
        ...
        if self.model_parallel:
            self._move_to_device(  # Custom device transfer logic
    ...
    def _compute_loss(self, logits, labels, use_pssm):
        if use_pssm:
            return self.loss_fct_chq(logits, labels)[0]  # Return only the loss value
